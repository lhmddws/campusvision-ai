...
export default defineComponent({
  ...
  props: {
    ...
    // 是否多选
    selection: VueTypes.bool.def(true),
    // 是否所有的超出隐藏，优先级低于schema中的showOverflowTooltip,
    showOverflowTooltip: VueTypes.bool.def(true),
    // 表头
    columns: {
    ...
    // 是否叠加索引
    reserveIndex: VueTypes.bool.def(false),
    // 对齐方式
    align: VueTypes.string
      .validate((v: string) => ['left', 'center', 'right'].includes(v))
      .def('left'),
    // 表头对齐方式
    headerAlign: VueTypes.string
      .validate((v: string) => ['left', 'center', 'right'].includes(v))
      .def('left'),
    data: {
