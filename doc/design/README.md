...
│
├── backend/                           ← Go 后端开发（搭档）
│   ├── 01-architecture.md             ← Go 架构设计：项目骨架、分层、配置、Kafka/Redis/调度
│   ├── 02-database.md                 ← 数据库设计：DDL、索引、分区、手动迁移、Redis Key
│   └── 03-api.md                      ← API 设计：22个端点完整请求/响应规格、错误码
│
├── integration/                       ← 系统集成（搭档）
│   └── 01-main-process.md            ← 主进程对接设计：Go模块提取、Handler迁移、认证合并、回退方案
│
├── camera/                            ← 摄像头功能实现（搭档）
...
| 角色                 | 负责内容                                    | 文档                                                          |
| -------------------- | ------------------------------------------- | ------------------------------------------------------------- |
| **Go 后端开发**      | 实现所有业务逻辑、数据库、API               | `backend/01-architecture.md` + `02-database.md` + `03-api.md` |
...
PRD-004 主进程对接
    ↓ 细化
backend/01-architecture.md  →  Gin 配置、分层、消费者
backend/02-database.md      →  11 张表 DDL、索引、手动迁移脚本
backend/03-api.md           →  22 个 REST 端点、错误码
    ↓
integration/01-main-process.md  →  独立模块 → 主进程模块的 8 步迁移
...
| 建表、写 Repository    | `backend/02-database.md` → DDL + 手动迁移           |
