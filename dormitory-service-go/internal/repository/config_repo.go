...
func (r *ConfigRepository) FindByKey(configKey string) (*entity.DormConfig, error) {
	var cfg entity.DormConfig
	query := "SELECT * FROM dorm_config WHERE config_key = ? LIMIT 1"
	err := r.DB.Get(&cfg, query, configKey)
	if err != nil {
...
func (r *ConfigRepository) FindByGroup(groupName string) ([]entity.DormConfig, error) {
	var configs []entity.DormConfig
	query := "SELECT * FROM dorm_config WHERE group_name = ? ORDER BY config_key"
	err := r.DB.Select(&configs, query, groupName)
	if err != nil {
...
func (r *ConfigRepository) UpdateByKey(configKey, configValue string) error {
	query := "UPDATE dorm_config SET config_value = ? WHERE config_key = ?"
	result, err := r.DB.Exec(query, configValue, configKey)
	if err != nil {
...
func (r *ConfigRepository) FindAllAsMap() (map[string]string, error) {
	configs, err := r.FindAll("config_key ASC")
	if err != nil {
		return nil, err
	}
