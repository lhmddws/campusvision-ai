...
**一句话定义**：从多路宿舍入口 RTSP 摄像头拉流（支持动态注册）→ FFmpeg 解码 → 抽帧 → Kafka 推送的实时流处理管道。
...
| 项目 | 规格 |
|------|------|
| 宿舍楼栋 | 4 栋（A/B/C/D 栋，可扩展） |
| 每栋摄像头 | 1 路起（入口处，仅判断进出，按需部署） |
| 摄像头总数 | **多路**（通过管理平台动态注册，无硬性上限） |
| 人员轨迹 | 仅判断两种状态：**进楼 / 出楼** |
| 核心产出 | 每晚宿舍在寝统计报表 |
...
| 职责 | 说明 |
|------|------|
| **拉流** | 同时管理多路宿舍入口 RTSP 摄像头流，支持断线自动重连 |
...
| **健康上报** | 摄像头在线状态（多路）、推流 FPS、内存/CPU 监控上报对接学管平台 |
...
| 配置管理 | YAML 文件 / 管理平台 | — | 摄像头配置（支持动态注册） |
...
| 属性 | 值 |
|------|-----|
| **功能描述** | 从多路宿舍入口 RTSP 摄像头拉取视频流。每路摄像头由独立 goroutine 管理，互不影响。配置通过 YAML 文件或管理平台下发，支持运行时动态注册摄像头。 |
| **优先级** | P0 |
| **输入** | `cameras[]` 动态数组，每项含：building（楼栋标识）, rtsp_url（RTSP 地址） |
...
| 4 路日均帧总数（示例） | ~1,728,000 | ~80,000-320,000 |
| 日均 Kafka 流量（4路示例） | ~173 GB | ~8-32 GB |
...
| 属性 | 值 |
|------|-----|
| **功能描述** | 使用 `building`（楼栋标识）作为 Kafka 消息 Key，确保同一栋楼的帧路由到同一 Partition。N 栋楼 → N 个 Key → Kafka 自动分配 Partition。保证 AI Engine 消费时同一栋楼的帧有序。 |
| **优先级** | P0 |
| **输入** | 帧消息 + building |
| **输出** | 已分区的 Kafka 消息 |
...
多路总带宽（以4路为例）：约 1-1.6 MB/s
日总量（4路示例）：约 84-138 GB（未压缩）
Kafka snappy 压缩后（4路示例）：约 50-80 GB
```
...
| 指标 | 目标值 | 说明 |
|------|--------|------|
| 最大拉流路数 | 可扩展（动态注册） | 取决于服务器资源与 Kafka 吞吐能力 |
...
| CPU 使用率（基准 4 路满负载） | ≤ 15%（单核） | 纯软件解码，路数增加后线性增长 |
...
|------|-----|------|-------------|-----------|
| 4 路 × 720p × 5fps × jpeg80（示例） | ≤ 15% 单核 | ≤ 128 MB | ≤ 2 MB/s | ~20 msg/s |
| 4 路 × 720p × 5fps × 动态抽帧（示例） | ≤ 5% 单核 | ≤ 128 MB | ≤ 0.5 MB/s | ~5 msg/s |
...
| 场景 | 单帧大小 | 单路日量 | 4 路日总量 |
|------|---------|---------|-----------|
| 白天正常帧（4路示例） | ~60 KB | ~26 GB | ~104 GB |
| 夜间红外帧（4路示例） | ~40 KB | ~17 GB | ~69 GB |
| 动态抽帧（日均，4路示例） | ~50 KB avg | ~4-16 GB | ~16-64 GB |
...
| 版本 | 日期 | 修改内容 | 作者 |
|------|------|---------|------|
| v2.1 | 2026-05-18 | 摄像头升级为可扩展管理平台：支持动态注册、加密凭据存储。移除固定 4 路限制。 | — |
| v2.0 | 2026-05-15 | 重构为宿舍管理子系统定位。移除通用安防能力，精简为多路可扩展配置、纯 CPU 解码、单 Kafka Topic、动态抽帧、低光照适配。简化配置清单和接口定义。 | — |
